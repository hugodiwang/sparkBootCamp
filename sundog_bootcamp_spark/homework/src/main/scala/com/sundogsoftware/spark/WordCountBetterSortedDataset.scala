...
object WordCountBetterSortedDataset {
  ...
  // we use name value here, because default column name for string is value, since it is a text here,
  // we do not have any schema actually
  ...
  def main(args: Array[String]) {
    ...
    // Show the results.
    // without the count.toInt, only 20 items are shown
